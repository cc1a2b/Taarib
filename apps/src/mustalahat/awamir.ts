...
export const commands = {
	 ...
	 * 
	 *  Artwork is deliberately **not** resolved here. The grid renders immediately
	 *  with plates and the cascade fills covers in afterwards; a scan that waited
	 *  on artwork would be a scan that waited on the network.
	 * 
	 *  # Errors
	 * 
	 ...
	 *  becomes an entry in [`HimayaHie::thughrat`].
	 */
	 ...
	 *  or the installer's own refusals — each in its own words.
	 */
	 ...
	 *  the real install pipeline refuses, in its own words.
	 */
 ...
 * 
 *  Neither `#[serde(default)]` nor a tolerant reader: a settings file that is
 *  missing a field is a damaged one, and quietly substituting the default is
 *  the failure this module exists to refuse — it is how somebody loses their
 *  provider setup without being told. A file written by an older build reaches
 *  this type only after `mukhattat` has migrated it forward, and a migration
 *  that introduces a field adds it explicitly, so every field is present by the
 *  time serde sees it. The absence of the attribute is also what makes the
 *  generated TypeScript honest: serialization always writes every field, so the
 *  interface receives every field, and `?` on any of them would be a lie the
 *  frontend then has to write code against.
 */
export type Idadat = {
	/**  Interface language. */
	lugha: Lugha,
...
export type IdadatMuzawwid = {
	...
	hadd_talabat: number,
	/**  A monthly spend ceiling in US dollars, enforced before each request. */
...
export type MintaqaHie = {
	...
	irtifa: number | null,
	/**  When it is read, as the interface slug spells it. */
	qaida: string,
...
export type ShaklMintaqa = {
	...
	irtifa: number | null,
	/**  The reading rule's committed slug. */
	qaida: string,
...
export type SijillMaktaba = {
	...
	/**  The resolved cover, or `None` when the plate is used instead. */
	ghilaf: string | null,
