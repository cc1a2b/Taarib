...
fn shakl_taqdimi(nass: &str) -> Option<char> {
    nass.chars()
        .find(|harf| matches!(u32::from(*harf), 0xFB50..=0xFDFF | 0xFE70..=0xFEFF))
}
...
fn renpy_yaktub_arabiyan_wa_yuidu_qiraatah() {
    ...
    assert!(
        !idad.contains("sajjil_khatt"),
        "no font supplied means no font registered:\n{idad}"
    );
    assert!(
        idad.contains("sajjil_ittijah()"),
        "the direction is always set:\n{idad}"
    );
    assert!(
        idad.contains("text_align 1.0"),
        "and so is the alignment:\n{idad}"
    );
    assert!(
        bayanat.contains("\"lugha\": \"arabic\""),
        "the settings name the language"
    );

    for (ayn, nass) in [
        ("taarib_hiwar.rpy", &hiwar),
        ("taarib_mustalahat.rpy", &mustalahat),
    ] {
        la_ashkal(nass, ayn);
    }
    ...
    assert!(
        thaniya
            .iter()
            .all(|sijill| sijill.asl != MARHABAN && sijill.asl != IBDA),
        "a second pass over a patched game must not re-ingest its own Arabic"
    );
    assert!(
        thaniya
            .iter()
            .all(|sijill| sijill.asl != "Bonjour, voyageur."),
...
fn renpy_yusajjil_alkhatt_alladhi_sayujad_fi_alluba() {
    ...
    let mawarid = Mawarid {
        tashghil_ghilaf: None,
        khatt_renpy: Some(KHATT_RENPY),
    };
...
fn rpgmaker_yaktub_arabiyan_fi_bayanatih() {
    ...
    let _ = jadwal.insert(
        " gold remains.".to_owned(),
        " قطعة ذهبية متبقية.".to_owned(),
    );
    ...
    assert!(
        taqreer.nusus >= 3,
        "at least the three plain lines were written: {taqreer:?}"
    );
    assert!(
        taqreer
            .mulahazat
            .iter()
            .any(|satr| satr.contains("escape code")),
    ...
    let mudawwar: Vec<&str> = sijill
        .madakhil
        .iter()
        .map(|madkhal| madkhal.khaam.as_str())
        .collect();
    assert!(
        mudawwar.contains(&MARHABAN),
        "the Arabic is what a re-read now finds"
    );
    assert!(mudawwar.contains(&JISR));
    assert!(
        !mudawwar.contains(&"Hello, traveller."),
        "and the English is gone"
    );
    ...
    assert!(
        asl.count() > 0,
        "the original data file was preserved before it was rewritten"
    );
}
...
fn ibni_data_win(nusus: &[&str]) -> Vec<u8> {
    ...
    assert_eq!(
        font.len(),
        HAJM_FONT,
        "the FONT chunk's size is what the layout assumed"
    );
...
fn gamemaker_yaktub_arabiyan_fi_hawd_nususih() {
    ...
    assert_eq!(
        qabl.khutut()[0].ashkal.len(),
        2,
        "and its baked glyph table as two glyphs"
    );
    ...
    assert_eq!(
        taqreer.matruka, 3,
        "the three resource names had no translation"
    );
    ...
    assert_eq!(
        baad.hawd().adad(),
        nusus.len(),
        "the pool's order and count never change"
    );
    ...
    assert_eq!(
        madakhil[3].nass, "spr_hero",
        "an untranslated entry is untouched"
    );
    ...
    assert_eq!(
        baad.ism_luba(),
        Some(DIFAF),
        "GEN8's name pointer was repointed, not left"
    );
    assert_eq!(
        baad.gen8().muarrif,
        0x0001_3579,
        "and nothing else in GEN8 moved"
    );
    ...
    assert_eq!(
        baad.khutut()[0].ism,
        "fnt_dialog",
        "a pointer into the pool that did not move"
    );
...
fn ibni_asar(madakhil: &[(&str, &[u8])]) -> Vec<u8> {
    ...
    for (masar, muhtawa) in madakhil {
        ...
        let _ = uqda.insert(
            "offset".to_owned(),
            serde_json::json!(ajsad.len().to_string()),
        );
...
fn ghilaf_yaktub_arabiyan_dakhil_asar() {
    ...
    assert!(
        taqreer.nusus >= 3,
        "every string the sweep found was translated: {taqreer:?}"
    );
    ...
    let hamula = baad
        .muhtawa("taarib/hamula.json")
        .expect("the payload is inside the archive");
    ...
    assert_eq!(
        qeema["jadwal"]["Hello, traveller."],
        serde_json::json!(MARHABAN)
    );
    ...
    assert_eq!(
        baad.muhtawa("index.js"),
        Some(&index[..]),
        "the application's own script"
    );
...
fn ayn_hadaf_yufaddil_almuharrik_aldakhili() {
    ...
    let saha = Saha::jadida();
    saha.iktub(
        "resources/app/js/rpg_core.js",
        b"Bitmap.prototype.drawText = function () {};\n",
    );
    saha.iktub(
        "resources/app.asar",
        &ibni_asar(&[("package.json", b"{\"main\":\"i.js\"}")]),
    );
    ...
    assert_eq!(
        aila.ism(),
        "RPG Maker MV",
        "the inner engine beats the wrapper"
    );
