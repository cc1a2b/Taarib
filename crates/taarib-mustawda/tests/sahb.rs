...
async fn al_tajdid_min_mujallad_mahalli_yanjah_wa_yukhazzan() -> NatijatIkhtibar {
    ...
    assert!(
        matches!(muraqaba.hala(), HalatQaima::Muhaddatha { .. }),
        "{:?}",
        muraqaba.hala()
    );
...
async fn mustawda_yujib_bila_qaima_yurfad_ala_al_bawwaba() -> NatijatIkhtibar {
    ...
    let natija =
        jaddid_qaimat_sahb(&silsila(jidhr), &masarat, &malik().aam(), &asliya()?, waqt).await;
    assert!(
        matches!(natija, NatijatTajdid::QaimaMutaadhdhira { .. }),
        "{}",
        natija.wasf()
    );
    ...
    let fashal = thabbit_bilnaqra(talab, |_muthabbit: &mut Nashir<'_>| Ok(()), |_marhala| {})
        .err()
        .ok_or("the pipeline must not install while the registry withholds its list")?;
    assert!(
        matches!(fashal, FashalTathbeet::Sahb(_)),
        "{}",
        fashal.injilizi()
    );
    assert!(
        fashal
            .injilizi()
            .contains("did not serve its revocation list"),
        "{}",
        fashal.injilizi()
    );
    assert!(
        fashal.arabi().contains("قائمة الإبطال"),
        "{}",
        fashal.arabi()
    );
    Ok(())
...
async fn mustawda_ghayr_mutah_la_yamnaa_wa_la_yaddai_al_fahs() -> NatijatIkhtibar {
    ...
    assert!(
        matches!(natija, NatijatTajdid::MustawdaGhayrMutah { .. }),
        "{}",
        natija.wasf()
    );
    ...
    assert!(
        ...
        "offline installs; it is never refused for being offline"
    );
    assert!(matches!(
        muraqaba.hala(),
        HalatQaima::LamTujlab { akhir: Some(_) }
    ...
    assert!(
        !muraqaba.hala().muhaddatha(),
        "and it is never told the check passed"
    );
    assert_eq!(
...
async fn qaima_aqdam_min_al_mahfudha_turfad_wa_tubqa_al_mahfudha() -> NatijatIkhtibar {
    ...
    assert_eq!(
        muraqaba.qaima().tasalsul(),
        3,
        "the replay must not un-revoke anything"
    );
...
async fn qaima_muntahiya_tuqbal_wa_tuqal_bila_rafd() -> NatijatIkhtibar {
    ...
    mustawda(
        &jidhr,
        MASAR_QAIMA,
        Some(&qaima(2, "2026-01-01T00:00:00Z")?),
    )?;
...
async fn bayan_talif_aw_rabt_kharij_yuad_qaima_mutaadhdhira() -> NatijatIkhtibar {
    ...
    mustawda(
        &kharij,
        "https://example.invalid/qaima.json",
        Some(&qaima(2, SALIH_HATTA)?),
    )?;
    let natija =
    ...
    let muraqaba = iqra(&masarat, waqt)?;
    assert!(
        muraqaba.rafd().is_some(),
        "an answering source with no usable list refuses"
    );
    Ok(())
}
