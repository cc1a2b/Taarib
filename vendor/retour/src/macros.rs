...
macro_rules! impl_hookable {
  ...
  (@impl_all ($($nm:ident : $ty:ident),*)) => {
    ...
    // TAARIB: `stdcall` and `fastcall` exist only on 32-bit x86. Rust made an
    // unsupported ABI a hard error (E0570), so these two lines stop the crate
    // compiling for every 64-bit target — which is every target Taarib ships.
    // Gated rather than deleted: on i686 they are still the right impls, and a
    // 32-bit game process is in the coverage matrix.
    #[cfg(target_arch = "x86")]
    impl_hookable!(@impl_pair ($($nm : $ty),*) (extern "stdcall"  fn($($ty),*) -> Ret));
    #[cfg(target_arch = "x86")]
    impl_hookable!(@impl_pair ($($nm : $ty),*) (extern "fastcall" fn($($ty),*) -> Ret));
    // TAARIB: the mirror image of the two lines above. `win64` exists only on
    // x86_64, so an ungated impl is the same E0570 with the architectures
    // swapped — it stops the crate compiling for `i686-pc-windows-msvc`, which
    // is the target every 32-bit game process needs and which rows B1/E1/F1/
    // G1/G2 of docs/tawzee.md §3 each ship a payload for.
    ...
    // TAARIB: `thiscall` is 32-bit x86 only, for the same reason.
